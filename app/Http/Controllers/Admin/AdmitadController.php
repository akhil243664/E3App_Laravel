...
class AdmitadController extends Controller
{
    ...
    public function index()
        ...
        $admin = Admin::find(auth('admin')->id());
         $admitad=DB::table('admitad')->where('country_slug', $admin->country_slug)->first();
       ...
       $headers =  array(
        ...
        if($token) {
           ...
           if(property_exists($res, 'error')){
               return redirect()->back()->withErrors('Check your admitad account credentials! Data Not Found.');
           }
            ...
            $getjoinoffers=Api_Advcampaign::where('country_slug',$admin->country_slug)->get();
            foreach($getjoinoffers as $offers){
                ...
                foreach ($res->results as $data) {
                    ...
                    if($data->status != 'active'){
                        $checkoffers=Api_Advcampaign::where('ad_id',$data->id)->where('country_slug',$admin->country_slug)->first();
                        
                        if($checkoffers){
                          $new_ad_id = $data->id; 
                          $checkoffers=Api_Advcampaign::where('ad_id',$data->id)->where('country_slug',$admin->country_slug)->update(['status'=>'0']);
                        }
                    }
                   ...
                   $flag = $check->country_code;
                    $resources[] = array_filter($data->action_countries, function ($var) use ($flag, $data,$admin) {
                        if ($var == $flag) {
                           ...
                           if($nammmmme != NULL){
                            $checkcategory=Category::where('name',$nammmmme)->where('country_slug',$admin->country_slug)->first();
                            if(!$checkcategory){
                            ...
                            $paar='.'.$partner_name;
                            $partner2=Partner::where('name',$paar)->where('affiliate_partner','admitad')->where('country_slug',$admin->country_slug)->first();
                            if($partner2){
                            ...
                            $partner1=Partner::where('name',$partner_name)->where('affiliate_partner','admitad')->where('country_slug',$admin->country_slug)->first();
                            if(!$partner1){ 
                            $adv= new Partner();
                            $adv->name=$partner_name;
                            $adv->status=1 ;
     ...
     public function couponfetch()
       ...
       $headers =  array(
        ...
        if($token) {
            ...
            $url = "https://api.admitad.com/coupons/website/".$this->website_id."/?limit=1000" ;
                ...
                foreach($res->results as $data){
                    if($data->status != 'active'){
                        $checkoffers=Coupon::where('admi_coupon_id',$data->id)->where('country_slug',$admin->country_slug)->first();
                        if($checkoffers){
                          $checkoffers=Coupon::where('admi_coupon_id',$data->id)->where('country_slug',$admin->country_slug)->where('status','0')->first();
                        }
                    }
                   ...
                   $flag = $check->country_code;
                       ...
                       $resources[] = array_filter($data->regions, function ($var) use ($flag, $data,$admin) {
                        ...
                        if ($var == $flag) { 
                           ...
                           if(!$checkoffers){
                            ...
                            $check1111= Api_Advcampaign::where('ad_id',$data->campaign->id)->where('country_slug',$admin->country_slug)->first();  
                           if($check1111){
                            $image = file_get_contents($url);
