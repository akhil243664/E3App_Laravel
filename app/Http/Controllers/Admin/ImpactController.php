...
function index(Request $request)
    ...
    $admin = Admin::find(auth('admin')->id());
	 $key = [];
        ...
        })->orderBy('id','desc')->where('country_slug',$admin->country_slug)->where('affiliate_partner','impact')->paginate(15);
  ...
  public function get_categories(Request $request)
    ...
    function list(Request $request)
    {
        ...
        ->with(['partner'])->where('affiliate_partner','impact')->where('country_slug',$admin->country_slug)->orderBy('id','desc')->paginate(15);
	...
	public function searchadvertisers(Request $request){
         $admin = Admin::find(auth('admin')->id());
        $key = explode(' ', $request['search']);
        $partners=Partner::where('country_slug',$admin->country_slug)->where('affiliate_partner','impact')->where(function ($q) use ($key) {
            foreach ($key as $value) {
    ...
    public function exportads(Request $request){
         $admin = Admin::find(auth('admin')->id());
        $ads = Offer::where('country_slug',$admin->country_slug)->where('affiliate_partner','impact')->get();
        $storage = [];
        if(count($ads)>0){
         foreach($ads as $item)
        {
      ...
      public function exportadvertisers(Request $request){
         $admin = Admin::find(auth('admin')->id());
        $adv = Partner::where('country_slug',$admin->country_slug)->where('affiliate_partner','impact')->get();
        $storage = [];
        if(count($adv)>0){
         foreach($adv as $item)
        {
