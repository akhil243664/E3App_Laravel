...
class CategoryController extends Controller
{
    function index()
    {
      $admin = Admin::find(auth('admin')->id());
        $categories=Category::where(['position'=>0])->latest()->where('country_slug',$admin->country_slug)->paginate(15);
	 ...
	 function liste(Request $request)
    {
        ...
        ->orderBy('id','desc')->where('country_slug',$admin->country_slug)->where(['position'=>0])->latest()->paginate(10);
        return view('admin.category.list',compact('categories'));
    }
	 function sub_list()
    {
        $admin = Admin::find(auth('admin')->id());
        $categories=Category::with(['parent'])->where(['position'=>1])->latest()->where('country_slug',$admin->country_slug)->paginate(15);
 ...
 public function update_cat_img(Request $request, $id)
    ...
    public function searchcategory (Request $request){
      ...
      public function exportcategory(Request $request){
        $admin = Admin::find(auth('admin')->id());
        $category = Category::where('country_slug',$admin->country_slug)->get();
        $storage = [];
        if(count($category)>0){
