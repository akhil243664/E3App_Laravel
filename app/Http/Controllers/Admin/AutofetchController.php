...
$brands=$data->campaigns;	
			...
			$getjoinoffers=CuelinkCampaign::where('country_slug',$admin->country_slug)->get();
	...
	foreach($brands as $brand){
       $partner=CuelinkCampaign::where('campaign_id',$brand->id)->where('country_slug',$admin->country_slug)->first();
		if(!$partner){
			$ch2 = curl_init();
...
if (preg_match('/^data:image\/(\w+);base64,/', $base64_image)) {
		 ...
		 $partner1=Partner::where('name',$brand->domain)->where('country_slug',$admin->country_slug)->first();
		if(!$partner1){	
	    $adv= new Partner();
        ...
        $cat=Category::where('cue_cat_id',$brand->categories[0]->id)->where('country_slug',$admin->country_slug)->first();
		if(!$cat){
...
$brands=$data->offers;	
 $get_id=NULL;
			...
			$getjoinoffers=CuelinkOffer::where('country_slug',$admin->country_slug)->get();
			foreach($getjoinoffers as $offers){
	...
	$delete=0;
	foreach($brands as $brand){
       $partner=CuelinkOffer::where('offer_id',$brand->id)->where('country_slug',$admin->country_slug)->first();
...
foreach($allKeysOfEmployee as &$tempKey){
	$tempKey=$tempKey;
        ...
        $cat=Category::where('cue_cat_id',$tempKey)->where('country_slug',$admin->country_slug)->first();
		if(!$cat){
			$category= new Category();
        ...
        $count++;
				...
				$coupon->image = $brand->id.".png";
				$coupon->url=$brand->url;
				$coupon->adv_id = NULL;
				$coupon->country_slug=$admin->country_slug;
		...
		if($partner && (strtotime($partner->end_date)< strtotime(date('Y-m-d'))) || ($brand->status != "live")){
			$partdelete=CuelinkOffer::where('offer_id',$brand->id)->where('country_slug',$admin->country_slug)->update(['status'=>0]);
			$delete++;
			if($brand->coupon_code != "" && $brand->coupon_code != NULL){
				$partd=Coupon::where('code',$brand->coupon_code)->where('country_slug',$admin->country_slug)->update(['status'=>0]);
				$update2++;
			}
		}
...
$brands=$data->Campaigns;		
		$count=0;
         $update=0;
	foreach($brands as $brand){
       $partner=Partner::where('adv_id',$brand->AdvertiserId)->where('country_slug',$admin->country_slug)->first();
...
if (preg_match('/^data:image\/(\w+);base64,/', $base64_image)) {
       ...
       if($partner && $brand->ContractStatus != "Active"){
			$partner=Partner::where('adv_id',$brand->AdvertiserId)->where('country_slug',$admin->country_slug)->update(['status'=>0]);
			$update++;
		}
...
$get_id=NULL;
			...
			$getjoinoffers=Offer::where('country_slug',$admin->country_slug)->get();
			foreach($getjoinoffers as $offers){
		...
		$count=0;
         $update=0;
	foreach($brands as $brand){
       $partner=Offer::where('ad_id',$brand->Id)->where('country_slug',$admin->country_slug)->first();
		 $partnercheck=Partner::where('adv_id',$brand->AdvertiserId)->where('country_slug',$admin->country_slug)->first();
		if($partnercheck){
