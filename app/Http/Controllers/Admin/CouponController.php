...
class CouponController extends Controller
{
    function index()
    {
        ...
        $coupons=Coupon::where('country_slug',$admin->country_slug)->paginate(15);
	 ...
	 function list(Request $request)
    {
	 ...
	 $key = [];
       ...
       $coupons=Coupon::where('country_slug',$admin->country_slug)->when(count($key) > 0, function($query)use($key){
    ...
    function store(Request $request)
    {
        ...
        $partner=Partner::where('country_slug',$admin->country_slug)->where('id',$request->adv_id)->first();
        $coupon = new Coupon();
    ...
    public function delete(Request $request)
    {
     ...
     public function exportcoupon(Request $request){
        $admin = Admin::find(auth('admin')->id());
        $coupon = Coupon::where('country_slug',$admin->country_slug)->get();
        $storage = [];
        if(count($coupon)>0){
