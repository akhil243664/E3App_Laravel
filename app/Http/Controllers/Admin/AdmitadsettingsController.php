...
class AdmitadsettingsController extends Controller
{
	 ...
	 public function check(Request $request)
    {
       ...
       $headers =  array(
        ...
        if(property_exists($re,'access_token')){
		    DB::table('admitad')->where('country_slug',$admin->country_slug)->update(['verified'=>1]);
			return redirect()->back()->withSuccess('Verified');
	
			

	   }else{
			  DB::table('admitad')->where('country_slug',$admin->country_slug)->update(['verified'=>2]);
    ...
    public function update(Request $request)
    {
       ...
       $admitadcheck=DB::table('admitad')->where('country_slug',$admin->country_slug)->first();
       if($admitadcheck){
        DB::table('admitad')->where('country_slug',$admin->country_slug)->update([
			'client_id'=>$client_id,
