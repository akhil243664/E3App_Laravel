...
function index(Request $request)
	...
	function listadv(Request $request)
    {
	 ...
	 $key = [];
        ...
        })->orderBy('id','desc')->where('affiliate_partner','cuelinks')->where('country_slug',$admin->country_slug)->paginate(15);
    ...
    public function coupan(Request $request){
       ...
       $coupons=Coupon::when(count($key) > 0, function($query)use($key){
        ...
        })->orderBy('id','desc')->where('affiliate_partner','cuelink')->where('country_slug',$admin->country_slug)->paginate(15);
  ...
  public function get_categories(Request $request)
    ...
    {
        $e = Offer::where('country_slug',$admin->country_slug)->where(['id' => $id])->first();
        $rls = Partner::where('country_slug',$admin->country_slug)->get();
    ...
    public function exportoffers(Request $request){
         $admin = Admin::find(auth('admin')->id());
        $offers = CuelinkOffer::where('country_slug',$admin->country_slug)->get();
        $storage = [];
        if(count($offers)>0){
 ...
 public function exportcampaign(Request $request){
        $admin = Admin::find(auth('admin')->id());
        $campaign = CuelinkCampaign::where('country_slug',$admin->country_slug)->get();
        $storage = [];
        if(count($campaign)>0){
 ...
 public function exportadvertisers(Request $request){
        $admin = Admin::find(auth('admin')->id());
        $adv = Partner::where('country_slug',$admin->country_slug)->where('affiliate_partner','cuelinks')->get();
        $storage = [];
        if(count($adv)>0){
         foreach($adv as $item)
        {
