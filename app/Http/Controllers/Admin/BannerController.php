...
class BannerController extends Controller
{
    function index()
    {
        ...
        $banners=Banner::where('country_slug',$country_slug)->paginate(15);
		$offers=CuelinkOffer::where('country_slug',$country_slug)->get();
    ...
    function store(Request $request)
    {
        ...
        $banner->country_slug = $admin->country_slug;
    ...
    public function edit($id)
    {
        $admin = Admin::find(auth('admin')->id());
        $country_slug=$admin->country_slug;
        $banner = Banner::where('country_slug',$country_slug)->findOrFail($id);
		$offers=CuelinkOffer::where('country_slug',$country_slug)->get();
   ...
   public function exportbanners(Request $request){
        $admin = Admin::find(auth('admin')->id());
        $ad= Banner::where('country_slug', $admin->country_slug)->get();
        $storage = [];
        if(count($ad)>0){
         foreach($ad as $item)
        {
