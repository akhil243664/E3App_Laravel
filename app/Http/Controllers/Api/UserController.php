...
class UserController extends Controller
{
    ...
    public function add_bank_detail(Request $request)
    {
		...
		$bankdetails=BankDetail::where('user_id',$request->user_id)->where('country_slug', $country_slug)->first();
		if($bankdetails){
					...
					$bank->country_slug = $country_slug;	
...
$country_slug= $request->header('countryslug') ?? 'in';;
	 ...
	 public function get_tracking_link(Request $request)
    {
       ...
       $headers =  array(
        ...
        if($token) {
             ...
             $check = Api_Advcampaign::where('ad_id',$request->c_id)->where('gotourl',$request->url)->first();
            
            $url = "https://api.admitad.com/deeplink/".$this->website_id."/advcampaign/".$request->c_id."/?subid=".$request->user_id."&ulp=".$check->site_url;
	...
	public function add_click(Request $request)
    {
		...
		$earning=Earning::where('user_id',$request->user_id)->where('country_slug',$user->country_slug)->first();
		if(!$earning){
			$earning=new Earning();
			$earning->user_id=$request->user_id;
			$earning->country_slug=$user->country_slug;
			$earning->pending_earnings=0;
			$earning->total_earnings=0;
			$earning->withdrawal=0;
			$earning->rem_earning=0;
			$earning->sent_for_withdrawal=0;
			$earning->referral_earning=0;
			$earning->reward_earning=0;
			...
			$earning->save();
					...
					$click->country_slug= $user->country_slug;
	...
	public function get_click(Request $request)
    {
		...
		$click = Click::where('user_id',$request->user_id)->where('country_slug',$user->country_slug)->orderBy('id','desc')->get();
	...
	public function send_withdrawal_request(Request $request)
    {
		...
		$country=Country::where('slug',$user->country_slug)->first();
		...
		$withdrawals->country_slug=$user->country_slug;
		$withdrawals->medium_details=$details;
		$withdrawals->approved=0;
		$withdrawals->order_id=$orderid;
		$withdrawals->merchant_id=$merchantid;
		$withdrawals->utr=$utr;
